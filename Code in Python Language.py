...
def signup():
    ...
    database[email] = {"name": name, "password": password, "mobile": mobile, "address": address, "pincode": pincode}
...
class ComboOffers:
    def __init__(self):
        self.offers = {
            "Biryani (Chicken)": {"quantity": 2, "free": "Coke"},
            "Fried Rice (Chicken)": {"quantity": 2, "free": "Spring Roll"},
            "Panner Dosa": {"quantity": 3, "free": "Idli"},
            "Chocolate Fudge": {"quantity": 2, "free": "Vanilla Ice Cream"},
            "Mixed Mandi (Chicken)": {"quantity": 1, "free": "Pepsi"},
            "Veg Biryani": {"quantity": 2, "free": "Raita"},
            "Grilled Chicken": {"quantity": 1, "free": "Salad"},
            "Butter Dosa": {"quantity": 3, "free": "Sambar"},
            "Lemon Rice": {"quantity": 2, "free": "Pickle"},
            "Juicy Mandi (Mutton)": {"quantity": 1, "free": "Water Bottle"},
            "Chicken Curry": {"quantity": 1, "free": "Naan"},
            "Coffee": {"quantity": 2, "free": "Donut"}
        }

    def apply_offers(self, cart_summary):
        applied_offers = []
        for item, offer in self.offers.items():
            if item in cart_summary and cart_summary[item]['quantity'] >= offer['quantity']:
                print(f"Combo Offer: Free {offer['free']} added!")
                applied_offers.append(item)
...
def view_cart():
    ...
    for item_name, details in cart.items():
        print(f"{item_name} x {details['quantity']} - Rs.{details['quantity'] * details['price']}")
    ...
    print(f"Total Amount after Discount: Rs.{total_amount}")
    return total_amount
...
def rating():
    ...
    as_food = int(input("Rate your Experience for the Food (1-5): "))
    if as_food == 5:
    ...
    as_hotel = int(input("Rate your Experience for the Hotel (1-5): "))
    if as_hotel == 5:
        print("Thank you for the Five Star Rating!")
    elif as_hotel == 4:
        print("Thank you for the Four Star Rating!")
...
def print_bill(email, payment_details):
    if email not in database:
    ...
    print(f"Email: {email}")
    print(f"Delivery Address: {user['address']}, Pincode: {user['pincode']}")
    ...
    print(f"Total: Rs.{total - discount}")
    
    if payment_details:
        if payment_details['mode'] == 'COD':
            print("Payment Mode: Cash on Delivery")
        elif payment_details['mode'] == 'Card':
            print(f"Payment Mode: Card (Last 4 digits: {payment_details['last_digits']})")
    else:
        print("Payment details unavailable.")
            ...
            if user_email:
                while True:
                    ...
                    if user_choice == '1':
                        ...
                        if 'payment_details' not in locals():
                            print("Payment is required before generating the bill.")
                        else:
                            print_bill(user_email, payment_details)
                    elif user_choice == '8':
